...
# ---------------------------------------------------------------
# عکس‌های پیش‌فرض بر اساس «نوع ملک»، وقتی ستون «عکس» در شیت خالی باشد.
...
DEFAULT_IMAGES = {
    ...
    "ویلا": "assets/defaults/villa.jpg",
    "باغ ویلا": "assets/defaults/bagh-villa.jpg",
    "باغ": "assets/defaults/bagh.jpg",
    "زمین": "assets/defaults/land.jpg",
    "مغازه": "assets/defaults/shop.jpg",
    "تجاری": "assets/defaults/commercial.jpg",
    "اداری": "assets/defaults/office.jpg",
    "کلنگی": "assets/defaults/kolangi.jpg",
    # اگر نوع ملک در بالا نبود یا خالی بود، از این عکس عمومی استفاده می‌شود:
    "_default": "assets/defaults/generic.jpg",
}


def get_default_image(property_type: str) -> str:
    """عکس پیش‌فرض متناسب با نوع ملک؛ اگر نوع ملک شناخته‌شده نبود، عکس عمومی."""
    pt = (property_type or "").strip()
    return DEFAULT_IMAGES.get(pt) or DEFAULT_IMAGES.get("_default", "")
...
def row_to_property(row: dict, deal_type: str) -> dict:
    ...
    # --- عکس: از شیت بخوان، اگر خالی بود عکس پیش‌فرض بر اساس نوع ملک بگذار ---
    image_url = _safe_get(row, "عکس")
    is_default_image = False
    if not image_url:
        image_url = get_default_image(result["property_type"])
