...
async function generateStoryImage(p) {
  try {
    ...
    cardInner += 4;  // divider
    cardInner += 32;
    if (hasSpecs) cardInner += 100 + 28;
    cardInner += 64; // addr
    if (hasExtras) cardInner += 26 + 60;
    cardInner += 36;
    cardInner += 168; // price
    cardInner += 36;
    cardInner += 108; // phone
    ...
    ctx.fillStyle = T.ink;
    setFont(ctx, 800, 64);
    rtlText(ctx, p.property_type || "ملک", cx, y + 48);
    y += 76 + 24;
    ...
    if (hasSpecs) {
      ...
      ctx.fillStyle = T.ink;
      setFont(ctx, 700, 42);
      rtlText(ctx, specs.join("   ·   "), cx, y + boxH / 2 + 15);
      y += 100 + 28;
    }
    ...
    rtlText(ctx, priceText, cx, y + 116);
    y += 168 + 40;

    // phone
    ctx.fillStyle = T.ink;
    setFont(ctx, 700, 32);
    rtlText(ctx, "📞  تماس با دفتر اطلس", cx, y + 30);

    ctx.fillStyle = T.sale || "#2F6B4F";
    setFont(ctx, 800, 46);
    ctx.direction = "ltr";
    ctx.textAlign = "center";
    ctx.fillText("0910 694 3220", cx, y + 90);
    ctx.direction = "rtl";
    y += 108;
    ...
    ctx.fillStyle = T.brass;
    setFont(ctx, 700, 28);
    rtlText(ctx, "مشاهده جزئیات و آگهی‌های مشابه", cx, fy + 8);
    fy += 32 + 36;
